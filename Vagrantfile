	 ...
	 config.vm.define "eduard2" do |vm1|
		...
		vm1.vm.provision "shell", inline: <<-SHELL
			...
			sudo echo '192.168.56.21 eduard3' >> /etc/hosts
                	...
                	echo 'server = eduard1' >>  /etc/puppetlabs/puppet/puppet.conf
                	echo 'runinterval = 160' >>  /etc/puppetlabs/puppet/puppet.conf
                	sudo systemctl start puppet
                	sudo systemctl enable puppet
...
#			sudo puppet module install puppetlabs-apt -v 4.2.0 --force --verbose --debug
                        sudo touch /etc/puppetlabs/code/environments/production/manifests/site.pp
#			sudo echo ~/site.pp >> /etc/puppetlabs/code/environments/production/manifests/site.pp

#			mkdir /etc/puppetlabs/code/environments/production/manifests/site.pp
#			sudo echo /Users/macbook/newvagrantfile/site.pp >> /etc/puppetlabs/code/environments/production/manifests/site.pp			

#			sudo scp -rp ~/newvagrantfile/site.pp >> /etc/puppetlabs/code/environments/production/manifests/site.pp

#			scp -i ~/newvagrantfile/site.pp /etc/puppetlabs/code/environments/production/manifests
#			vm2.vm.provision "file", source: "~/newvagrantfile/site.pp", destination: "/etc/puppetlabs/code/environments/production/manifests/site.pp"

#                	sudo /opt/puppetlabs/bin/puppet agent --test

#			scp site.pp etc/puppetlabs/code/environments/production/manifests/ 

#			vm2.vm.provision "file", source:"~/newvagrantfile/site.pp", destination:"~/etc/puppetlabs/code/environments/production/manifests/site.pp"

            SHELL
	
	  vm2.vm.provision "file", source: "~/Users/macbook/newvagrantfile/site.pp", destination: "~/etc/puppetlabs/code/environments/production/manifests/site.pp"



#	  vm2.vm.provision "shell", inline: <<-SHELL
#         	echo ~/newvagrantfile/site.pp >> /etc/puppetlabs/code/environments/production/manifests/site.pp
